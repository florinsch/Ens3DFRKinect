...
namespace face_detection_apps_utils
{
  ...
  void displayHeads(std::vector<Eigen::VectorXf> & heads, pcl::visualization::PCLVisualizer & vis, int viewport)
  {
    for (size_t i = 0; i < heads.size (); i++)
    {
      ...
      ctr_pt.y = heads[i][1];
      ctr_pt.z = heads[i][2];
      vis.addSphere (ctr_pt, 0.02, 255, 255, 0, name.str (), viewport);

      pcl::ModelCoefficients cylinder_coeff;
      cylinder_coeff.values.resize (7); // We need 7 values
      cylinder_coeff.values[0] = ctr_pt.x;
      cylinder_coeff.values[1] = ctr_pt.y;
      cylinder_coeff.values[2] = ctr_pt.z;

      Eigen::Vector3f vec = Eigen::Vector3f::UnitZ () * -0.3f;
      ...
      cylinder_coeff.values[6] = 0.003f;
      name << "cylinder";
      vis.addCylinder (cylinder_coeff, name.str (), viewport);
      
      // put a cube around the head
      pcl::ModelCoefficients cube_coeff = cylinder_coeff;
      cube_coeff.values.resize (10);
      
      cube_coeff.values[3] = 0;//vec[0]; //Qx
      cube_coeff.values[4] = 0;//vec[1]; //Qy
      cube_coeff.values[5] = 0;//vec[2]; //Qz
      
      cube_coeff.values[6] = 0.0f; //Qw cube rotation
      cube_coeff.values[7] = 0.18f; //depth
      cube_coeff.values[8] = 0.18f; //height
      cube_coeff.values[9] = 0.18f; //depth
      
      name << "head_cube" << i;
      vis.addCube (cube_coeff, name.str (), viewport);
    }
  }
}
